...
def main(config, specs, raw_data_path, reduced_data_path, times, **kwargs):
    ...
    energy flux at given times over radius.

    Energy flux is defined in Eq. 7 of White, Quataert, and Gammie 2020
    ...
    for time in times:
        ...
        # calculate T^1_0
        Trt = calculate_Trt((four_vel, proj_b, coords))
        energy_flux_over_r = calculate_flux_over_radial_shells((Trt, coords))

        reduced_data_fp = reduced_data_path + "norm_energy_flux_over_r_at_t{:05}.txt".format(time)
        norm_energy_flux = energy_flux_over_r/mass_flux_over_r

        if write_to_file:
            x1vstr = ', '.join(map(str, x1v))
            np.savetxt(reduced_data_fp, norm_energy_flux, header=x1vstr)


        label = "$t={:.2f}~GM/c^3$".format(sim_time)

        # ----- Now the actual plotting ------
        plt.plot(x1v, norm_energy_flux, marker="o", markersize=3, label=label)

    plt.xlabel(r"$r/r_g$")
    plt.ylabel(r"$\dot E/\dot M$")
    titstr = "Energy flux (normalized to mass flux) through each radial shell"
    plt.legend()
    plt.title(titstr)
    plt.tight_layout()
    plt.show()
